...
public interface ReviewApi {
  ...
  ResponseEntity<CursorPageResponseDto<ReviewDto>> getReviews(
      ...
      @Parameter(description = "커서: 마지막으로 본 리뷰의 생성일시 (YYYY-MM-DDThh:mm:ss)")
      @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime after,
      @Parameter(description = "정렬 기준 (LATEST 또는 RATING)") @RequestParam(defaultValue = "LATEST") String sortBy,
      // @Parameter(description = "커서: 마지막으로 본 리뷰의 평점 (RATING 정렬 시 필수)") @RequestParam(required = false) Integer rating,
      @Parameter(description = "로그인한 유저 ID (좋아요 여부 확인용)") @RequestHeader(value = "Deokhugam-Request-User-ID", required = false) UUID requestUserId
  );
  ...
  ResponseEntity<ReviewDto> getReviewDetail(
      @Parameter(description = "리뷰 ID") @PathVariable UUID reviewId,
      @Parameter(description = "유저 ID (좋아요 확인용)") @RequestHeader(value = "X-User-Id", required = false) UUID userId
  );

  @Operation(summary = "리뷰 물리 삭제", description = "DB에서 리뷰를 완전히 삭제합니다.")
  ResponseEntity<Void> hardDeleteReview(
      @Parameter(description = "리뷰 ID") @PathVariable UUID reviewId
      //  @RequestHeader("X-User-Id") 삭제됨
  );
}